...
function fixture({changed=false,occupied=false,status=0,owner='ours',delay=0}={}) {
 let calls=0,reads=0,waits=0,waitedMs=0;
 return {get calls(){return calls;},get reads(){return reads;},get waits(){return waits;},get waitedMs(){return waitedMs;},
 latest:()=>({version:changed?'1.0.2':'1.0.0',gitHead:'old'}),
 versions:()=>occupied?['1.0.0','1.0.1']:['1.0.0'],
 publish:()=>{calls++;return status;},wait:async(ms)=>{waits++;waitedMs+=ms;},
...
test('persistent registry failure is bounded and never republishes',async()=>{const io=fixture({status:1,delay:99});await assert.rejects(publishRelease(input,io),/Could not verify/);assert.equal(io.calls,1);assert.equal(io.reads,61);assert.equal(io.waitedMs,300000);});
test('visibility after five minutes succeeds without republishing',async()=>{const io=fixture({delay:60});assert.equal((await publishRelease(input,io)).published,true);assert.equal(io.calls,1);assert.equal(io.waitedMs,300000);});
